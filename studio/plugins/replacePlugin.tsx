...
// Triggers on shift+mod+r
export const replacePlugin = definePlugin({
  name: '@sanity/fix-spelling',

  form: {
    components: {
      input: function ReplacePlugin(props: InputProps) {
        ...
        const focused = useMemo(() => {
          return (
            (!focusPath.length && props.id === 'root') ||
            componentFocused ||
            (getPortableTextBlock(fieldSchemaType) &&
              !!(focusPath.length && path.length && pathStartsWith(focusPath, path)))
          )
        }, [path, focusPath, fieldSchemaType, componentFocused, props.id])

        const replaceWhatWeDonWant = useCallback(
          (event: KeyboardEvent) => {
            const isModPressed = event.metaKey || event.ctrlKey

            if (!(event.shiftKey && isModPressed && event.key === 'r')) {
              return false
            }
            ...
            client.agent.action
              .transform({
                schemaId,
                documentId: targetId,
                instruction: `Replace any occurrence of Contentful with Sanity, leave everything else as is.`,
          ...
          [path, client, schemaId, documentSchemaType, documentId, toast],
        )

        useEffect(() => {
          if (!focused) return
          window.addEventListener('keydown', replaceWhatWeDonWant, {capture: true})
          return () => window.removeEventListener('keydown', replaceWhatWeDonWant, {capture: true})
        }, [replaceWhatWeDonWant, focused])
